...
let interp_context_gen scope ~program_mode ~poly ~kind ~autoimp_enable ~coercions env sigma l =
  ...
  let sigma, ctx = Evarutil.finalize ~to_type:(not poly) ~partial:true sigma @@ fun nf ->
...
let interp_context env sigma ctx =
  ...
  let sigma, ctx = interp_context_gen Locality.Discharge ~program_mode:false ~poly:false ~kind:Context ~autoimp_enable:false ~coercions:Id.Set.empty env sigma ctx in
