...
let build_inductive env ~sec_univs names prv univs template
  ...
  in
  let record_info = match is_record with
  | Some (Some rid) ->
    (** The elimination criterion ensures that all projections can be defined. *)
    let map i id =
      let labs, rs, projs = compute_projections (kn, i) mib in
      (id, labs, rs, projs, recheck_at_eta_conv)
    in
...
let check_inductive env ~sec_univs kn mie =
  ...
  let (env_ar_par, univs, template, record, why_not_prim_record, recheck_at_eta_conv, paramsctxt, inds) =
