...
Local Ltac not_dep_intros :=
  ...
  | |- (Coq.Init.Types.Empty.not _) => unfold Coq.Init.Types.Empty.not at 1; intro
  end.
...
Local Ltac simplif flags :=
  ...
  repeat
     (match reverse goal with
      ...
      | id: (Coq.Init.Types.Functions.iff _ _) |- _ => elim id; do 2 intro; clear id
      | id: (Coq.Init.Types.Empty.not _) |- _ => red in id
  ...
  (* moved from "id:(?A/\?B)->?X2|-" to "?A->?B->?X2|-" *)
      | id: forall (_: Coq.Init.Types.Functions.iff ?X1 ?X2), ?X3|- _ =>
  ...
  (* moved from "id:(?A\/?B)->?X2|-" to "?A->?X2,?B->?X2|-" *)
      ...
      | |- (Coq.Init.Types.Functions.iff _ _) => split
      | |- (Coq.Init.Types.Empty.not _) => red
      end;
      not_dep_intros).
