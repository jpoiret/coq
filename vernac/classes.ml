...
let declare_instance_constant iinfo global impargs ?hook (name:lident) udecl poly sort_poly cumulative sigma term termtype =
  let kind = Decls.(IsDefinition Instance) in
  let cinfo = Declare.CInfo.make ?loc:name.loc ~name:name.v ~impargs ~typ:(Some termtype) () in
  let info = Declare.Info.make ~kind ~poly ~cumulative ~udecl () in
  let kn = Declare.declare_definition ~cinfo ~info ~opaque:false ~sort_poly ~body:term sigma in
...
let do_instance_resolve_TC ~sort_poly term termtype sigma env =
  ...
  let sigma = Evd.minimize_universes ~to_type:(not sort_poly) ~partial:(Option.is_empty term) sigma in
...
let do_instance_interactive env env' sigma ?hook ~tac ~locality ~poly ~sort_poly ~cumulative cty k ctx ctx' pri decl imps subst id opt_props =
  let term, termtype, sigma = match opt_props with
    ...
    | None ->
      ...
      in
      let termtype, sigma = do_instance_resolve_TC ~sort_poly term termtype sigma env in
      term, termtype, sigma
  in
  Flags.silently (fun () ->
...
let do_instance env env' sigma ?hook ~locality ~poly ~sort_poly ~cumulative cty k ctx ctx' pri decl imps subst id props =
  let term, termtype, sigma =
    interp_props ~program_mode:false env' cty k ctx ctx' subst sigma props
  in
  let termtype, sigma = do_instance_resolve_TC ~sort_poly (Some term) termtype sigma env in
  Pretyping.check_evars_are_solved ~program_mode:false env sigma;
  declare_instance_constant pri locality imps ?hook id decl poly sort_poly cumulative sigma term termtype
...
let do_instance_program ~pm env env' sigma ?hook ~locality ~poly ~sort_poly ~cumulative cty k ctx ctx' pri decl imps subst id opt_props =
  ...
  if not (Evd.has_undefined sigma) && not (Option.is_empty opt_props) then
    let () = declare_instance_constant pri locality imps ?hook id decl poly sort_poly cumulative sigma term termtype in
    pm
  else
...
let interp_instance_context ~program_mode ~sort_poly env ctx pl tclass =
  let sigma, decl = interp_cumul_poly_decl_opt env pl in
  let sigma, (impls, ((env', ctx), imps, _locs)) = interp_context_evars ~program_mode ~unconstrained_sorts:sort_poly env sigma ctx in
...
let new_instance_interactive ~locality ~poly ~sort_poly ~cumulative instid ctx cl
...
let declare_new_instance ~locality ~program_mode ~poly ~sort_poly ~cumulative instid ctx cl pri =
  let env = Global.env() in
  let (instid, pl) = instid in
  let sigma, k, u, cty, ctx', ctx, imps, subst, decl =
    interp_instance_context ~program_mode ~sort_poly env ctx pl cl
