...
let do_definition ?loc ?hook ~name ?scope ?clearbody ~poly ~sort_poly ~cumulative ?typing_flags ~kind ?using ?user_warns udecl bl red_option c ctypopt =
  let program_mode = false in
  let env = Global.env() in
  let env = Environ.update_typing_flags ?typing_flags env in
  (* Explicitly bound universes and constraints *)
  let evd, udecl = interp_cumul_poly_decl_opt env udecl in
  ...
  let _ : Names.GlobRef.t =
    (* Declare.declare_definition ~info ~cinfo ~opaque:false ~sort_poly:(List.is_empty udecl.polydecl_qualities) ~body ?using evd *)
    Declare.declare_definition ~info ~cinfo ~opaque:false ~sort_poly:(List.is_empty udecl.polydecl_qualities && sort_poly) ~body ?using evd
  in ()

let do_definition_program ?loc ?hook ~pm ~name ~scope ?clearbody ~poly ~sort_poly ~cumulative ?typing_flags ~kind ?using ?user_warns udecl bl red_option c ctypopt =
  let env = Global.env() in
  let env = Environ.update_typing_flags ?typing_flags env in
  (* Explicitly bound universes and constraints *)
  let evd, udecl = interp_cumul_poly_decl_opt env udecl in
  ...
  in
  let body, typ, uctx, _, obls = Declare.Obls.prepare_obligations ~name ~sort_poly ~body ?types env evd in
  Evd.check_poly_decl_early ~poly ~sort_poly ~cumulative ~with_obls:true (Evd.from_ctx uctx) udecl [body; typ];
  let pm, _ =
    let cinfo = Declare.CInfo.make ?loc ~name ~typ ~impargs () in
    let info = Declare.Info.make ~udecl ~scope ?clearbody ~poly ~cumulative ~kind ?hook ?typing_flags ?user_warns () in
    Declare.Obls.add_definition ~pm ~info ~cinfo ~opaque:false ~body ~uctx ?using obls
  in pm

let do_definition_interactive ?loc ~program_mode ?hook ~name ~scope ?clearbody ~poly ~sort_poly ~cumulative ~typing_flags ~kind ?using ?user_warns udecl bl t =
  let env = Global.env () in
  let env = Environ.update_typing_flags ?typing_flags env in
  let flags = Pretyping.{ all_no_fail_flags with program_mode; sort_polymorphic = sort_poly } in
  ...
  let evd = Evd.minimize_universes ~to_type:(not sort_poly) ~partial:udecl.polydecl_extensible_instance evd in
  Pretyping.check_evars_are_solved ~program_mode env evd;
  let typ = EConstr.to_constr evd typ in
  Evd.check_poly_decl_early ~poly ~sort_poly ~cumulative ~with_obls:false evd udecl [typ];
  let typ = EConstr.of_constr typ in
  let info = Declare.Info.make ?hook ~poly ~sort_poly ~cumulative ~scope ?clearbody ~kind ~udecl ?typing_flags ?user_warns () in
