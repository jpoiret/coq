...
let register_struct is_rec (rec_order, fixpoint_exprl) =
    ...
    let body =
      ...
      | None ->
        CErrors.user_err
          Pp.(str "Body of Function must be given.")
    in
    ComDefinition.do_definition ?loc:fname.CAst.loc ~name:fname.CAst.v ~poly:false ~sort_poly:false ~cumulative:false
  ...
  | _ ->
    let pm, p = ComFixpoint.do_mutually_recursive ~refine:false ~program_mode:false ~poly:false ~sort_poly:false ~cumulative:false ~kind:(IsDefinition Fixpoint) (CFixRecOrder rec_order, fixpoint_exprl) in
