...
let has_valid_relevance u ind_relevance flds =
  ...
  | Sorts.Irrelevant -> true
  | Sorts.Relevant -> Array.for_all Sorts.is_relevant flds
  | Sorts.RelevanceVar qv ->
    Array.for_all (fun r -> match r with
        | Sorts.Relevant -> true
        | Sorts.Irrelevant -> false
        | Sorts.RelevanceVar qv' -> Quality.QVar.equal qv qv') flds
...
let eta_expand_ind_stack env (ind,u) m (f, s') =
  ...
  | Some projs, recheck ->
    (* let subst_relevances = Array.map (fun (_, r) -> UVars.subst_instance_relevance u r) projs in *)
    if recheck && not @@ has_valid_relevance u ind_relevance (Array.map snd projs)
  ...
  | None, (true | false) -> raise Not_found (* disallow eta-exp for non-primitive records *)
