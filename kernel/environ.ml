...
let add_universes ~strict ctx g =
  debug Pp.(fun () -> str"Adding universe context" ++ UVars.pr_universe_context Quality.QVar.raw_pr Univ.Level.raw_pr ctx);
...
let add_universes_set ~strict ctx g =
  debug Pp.(fun () -> str"Adding universes context" ++ PolyConstraints.ContextSet.pr Quality.QVar.raw_pr Univ.Level.raw_pr ctx);
  ...
  in fst (UGraph.merge_constraints (PolyConstraints.ContextSet.univ_constraints ctx) g)
