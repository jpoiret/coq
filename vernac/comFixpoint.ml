  ...
  let hook, impls =
    if len > 1 then
      let hook { Declare.Hook.S.dref; uctx; obls; _ } =
        ...
        let univs = UState.check_poly_decl ~poly ~cumulative ~kind:UVars.Definition uctx udecl in
        let h_body =
...
let interp_mutual_definition env ~program_mode ~poly ~function_mode rec_order fixl =
  ...
  let sigma = Evd.minimize_universes ~to_type:(not poly) ~partial:(List.exists Option.is_empty fixdefs) sigma in
...
let interp_fixpoint_short rec_order fixpoint_exprl =
  ...
  let (_, _, sigma),(fix, _, _) = interp_mutual_definition ~program_mode:false ~poly:false ~function_mode:true env (CFixRecOrder rec_order) fixpoint_exprl in
  (* Instantiate evars and check all are resolved *)
  let sigma = Evarconv.solve_unif_constraints_with_heuristics env sigma in
...
let do_mutually_recursive ?pm ~refine ~program_mode ?(use_inference_hook=false) ?scope ?clearbody ~kind ~poly ~cumulative ?typing_flags ?user_warns ?using (rec_order, fixl)
  ...
  | Some pm ->
    ...
    Evd.check_poly_decl_early ~poly ~cumulative ~with_obls:true sigma udecl (bodies @ fixtypes);
  ...
  | None ->
    ...
    with Option.IsNone ->
      ...
      Evd.check_poly_decl_early ~poly ~cumulative ~with_obls:false sigma udecl (Option.List.flatten bodies @ fixtypes);
      let possible_guard = (possible_guard, fixrs) in
