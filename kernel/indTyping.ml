...
let compute_elim_squash ?(is_real_arg=false) env out_sort info =
  ...
  if (Environ.type_in_type env) then info
    ...
    if Inductive.eliminates_to (Environ.qualities env) (Sorts.quality ind_sort) (Sorts.quality out_sort) then
      ...
      | QSort (q, indu), Set ->
        (* Morally, this checks if q is "above Prop",
...
let typecheck_inductive env ~sec_univs (mie:mutual_inductive_entry) =
  ...
  let env_univs =
    ...
    | Polymorphic_ind_entry (ctx, _variance) -> push_context QGraph.Internal ctx env
  in
  ...
  env_ar_par, univs, template, record, why_not_prim_record, recheck_at_eta_conv, params, Array.of_list data
