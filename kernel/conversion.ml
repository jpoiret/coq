...
let check_cumul_instances ~flex:_ ~nargs cv_pb variance u1 u2 (univs, elims) =
  let qcsts, ucsts = get_cumulativity_constraints cv_pb ~nargs variance u1 u2 in
  if QGraph.check_constraints qcsts elims && (UGraph.check_constraints ucsts univs) then Result.Ok (univs, elims)
  else Result.Error None
...
let conv_inst u1 u2 ((univs, _ : UGraph.t * QGraph.t) as gs) = if UGraph.check_eq_instances univs u1 u2 then Result.Ok gs else Result.Error ()
let cumul_inst ~nargs cv_pb variances u1 u2 (univs as gs) =
  if check_cumul_instances_univs ~flex:false cv_pb ~nargs variances u1 u2 univs then Result.Ok gs else Result.Error ()
