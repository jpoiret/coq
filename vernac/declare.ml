...
let extract_monomorphic entry =
  ...
  | UState.Polymorphic_entry (uctx, variances) -> Entries.Polymorphic_entry (uctx, variances), PolyConstraints.ContextSet.empty
...
let add_mono_uctx uctx e =
  ...
  | UState.Monomorphic_entry ctx ->
    { e with universes_entry_universes = UState.Monomorphic_entry (PolyConstraints.ContextSet.union (UState.context_set uctx) ctx) }
  | UState.Polymorphic_entry _ -> assert (PolyConstraints.ContextSet.is_empty (UState.context_set uctx)); e
...
let make_univs_immediate_private_poly ~cumulative ~uctx ~udecl ~eff ~used_univs body typ =
  ...
  let utyp = UState.check_poly_decl ~poly:true ~cumulative ~kind:UVars.Definition uctx' udecl in
  let ubody =
...
let make_univs_immediate_default ~poly ~cumulative ~opaque ~uctx ~udecl ~eff ~used_univs body typ =
  ...
  let uctx = UState.restrict uctx used_univs in
  let utyp = UState.check_poly_decl ~poly ~cumulative ~kind:UVars.Definition uctx udecl in
  let utyp = match utyp.universes_entry_universes with
    ...
    | Monomorphic_entry uctx ->
      ...
      { utyp with universes_entry_universes = Monomorphic_entry (PolyConstraints.ContextSet.union uctx (Safe_typing.universes_of_private eff.Evd.seff_private)) }
  in
...
let cast_pure_proof_entry (e : Constr.constr pproof_entry) =
  let univ_entry, cstrs = extract_monomorphic e.proof_entry_universes in
...
let declare_constant ~loc ?(local = Locality.ImportDefaultBehavior) ~name ~kind ~typing_flags ?user_warns cd =
  ...
  (* Logically define the constant and its subproofs, no libobject tampering *)
    | DefinitionEntry pe ->
      ...
      | Default { body = (body, eff); opaque = Transparent } ->
        ...
        let pe = { pe with proof_entry_body = body } in
        let def_entry, cstrs = cast_pure_proof_entry pe in
        let ubinders = make_ubinders cstrs pe.proof_entry_universes in
        (* We register the global universes after exporting side-effects, since
           the latter depend on the former. *)
        let ctx = PolyConstraints.ContextSet.filter_out_constant_qualities cstrs in
        let () = Global.push_context_set QGraph.Rigid ctx in
        Entries.DefinitionEntry def_entry, false, ubinders, None, ctx
    ...
    | PrimitiveEntry e ->
      ...
      | None ->
        None, default_named_univ_entry, PolyConstraints.ContextSet.empty
  ...
  let () =
    ...
    let is_new_lvl_constraint = is_new_constraint before_univs UGraph.check_declared_universes
                                                  UGraph.check_constraint (fun u1 u2 -> Univ.Level.Set.(union (Univ.Universe.levels u1) (Univ.Universe.levels u2))) in
    let is_new_quality_constraint = is_new_constraint before_qualities QGraph.check_declared_qualities
                                                      QGraph.check_constraint (fun q1 q2 -> Quality.Set.(add q1 (singleton q2))) in
    let ctx = on_snd (PolyConstraints.filter_constraints is_new_quality_constraint is_new_lvl_constraint) ctx in
    ...
    | SectionLocalDef { clearbody; entry = de } ->
      ...
      let univs = match peunivs.universes_entry_universes with
        | UState.Monomorphic_entry uctx ->
          ...
          UState.{ universes_entry_universes = UState.Monomorphic_entry PolyConstraints.ContextSet.empty; universes_entry_binders = UnivNames.empty_binders }
...
let declare_possibly_mutual_parameters ~info ~cinfo ?(mono_uctx_extra=UState.empty) ~sec_vars typs =
  ...
  pi3 (List.fold_left2 (
    fun (i, subst, csts) { CInfo.name; loc; impargs } (typ, uctx) ->
      ...
      let univs = UState.check_poly_decl ~poly ~cumulative ~kind:Assumption (Evd.ustate sigma) udecl in
