...
let do_symbol ~poly ~unfold_fix udecl (id, typ) =
  ...
  let flags = { Pretyping.all_no_fail_flags with polymorphic = poly } in
  let evd, (typ, impls) =
...
let interp_rule ~poly (udecl, lhs, rhs: Constrexpr.poly_decl_expr option * _ * _) =
  let env = Global.env () in
  ...
  let flags = { Pretyping.no_classes_no_fail_inference_flags with 
    ...
    solve_unification_constraints = false; polymorphic = poly } in
  ...
  let flags = { Pretyping.no_classes_no_fail_inference_flags with patvars_abstract = true; polymorphic = poly } in
  let evd', rhs =
