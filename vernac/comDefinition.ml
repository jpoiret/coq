...
let interp_statement ~program_mode env evd ~flags ~scope name bl typ  =
  let evd, (impls, ((env, ctx), imps, _locs)) = Constrintern.interp_context_evars ~poly:(Pretyping.(flags.polymorphic)) ~program_mode env evd bl in
...
let do_definition ?loc ?hook ~name ?scope ?clearbody ~poly ~cumulative ?typing_flags ~kind ?using ?user_warns udecl bl red_option c ctypopt =
  ...
  let _ : Names.GlobRef.t =
    (* Declare.declare_definition ~info ~cinfo ~opaque:false ~poly:(List.is_empty udecl.polydecl_qualities) ~body ?using evd *)
    Declare.declare_definition ~info ~cinfo ~opaque:false ~poly:(List.is_empty udecl.polydecl_qualities && poly) ~body ?using evd
  in ()
...
let do_definition_program ?loc ?hook ~pm ~name ~scope ?clearbody ~poly ~cumulative ?typing_flags ~kind ?using ?user_warns udecl bl red_option c ctypopt =
  ...
  Evd.check_poly_decl_early ~poly ~cumulative ~with_obls:true (Evd.from_ctx uctx) udecl [body; typ];
  let pm, _ =
...
let do_definition_interactive ?loc ~program_mode ?hook ~name ~scope ?clearbody ~poly ~cumulative ~typing_flags ~kind ?using ?user_warns udecl bl t =
  ...
  let evd = Evd.minimize_universes ~to_type:(not poly) ~partial:udecl.polydecl_extensible_instance evd in
  Pretyping.check_evars_are_solved ~program_mode env evd;
  let typ = EConstr.to_constr evd typ in
  Evd.check_poly_decl_early ~poly ~cumulative ~with_obls:false evd udecl [typ];
