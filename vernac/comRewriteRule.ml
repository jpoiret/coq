...
let interp_rule ~sort_poly (udecl, lhs, rhs: Constrexpr.poly_decl_expr option * _ * _) =
  let env = Global.env () in
  ...
  let flags = { Pretyping.no_classes_no_fail_inference_flags with patvars_abstract = true; sort_polymorphic = sort_poly } in
  let evd', rhs =
