...
let interp_fields_evars ~primitive_proj ~poly env sigma ~ninds ~nparams record_sort impls_env fld_notations flds =
...
let finalize_def_class ~poly env sigma ~params ~sort ~projtyp =
  let sigma, (params, sort, typ, projtyp) =
    Evarutil.finalize ~abort_on_undefined_evars:false ~to_type:(not poly) sigma ~partial:false (fun nf ->
...
let typecheck_params_and_fields ~kind ~(flags:ComInductive.flags) ~primitive_proj udecl params (records : DataI.t list) =
  ...
  let sigma, udecl = Constrintern.interp_cumul_poly_decl_opt env0 udecl in
  ...
  if def then
    ...
    let univs = Evd.check_poly_decl ~poly:flags.poly ~cumulative:flags.cumulative ~kind:UVars.Definition sigma udecl in
...
let declare_class_constant entry (data:Data.t) =
  ...
  let inst, univs = match univs.universes_entry_universes with
    | UState.Monomorphic_entry _ ->
      ...
      UState.{ univs with universes_entry_universes = UState.Monomorphic_entry PolyConstraints.ContextSet.empty }
    | UState.Polymorphic_entry (uctx, variances) ->
