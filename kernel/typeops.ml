...
let type_case_scrutinee env (mib, _mip) (u', largs) u pms (pctx, p) c =
  ...
  | Monomorphic | Polymorphic (_, None) -> UVars.enforce_eq_instances u u' PolyConstraints.empty
  | Polymorphic (_, Some variance) -> UVars.enforce_leq_variance_instances ~nargs:UVars.FullyApplied variance u' u PolyConstraints.empty
  in
