...
let pr_puniverses p u =
  ...
  else Pp.(p ++ str"(*" ++ UVars.Instance.pr Quality.QVar.raw_pr (Univ.Universe.pr Univ.Level.raw_pr) u ++ str"*)")
...
let rec debug_print c =
  ...
  | Case (_ci,_u,pms,(p,_),iv,c,bl) ->
    ...
    v 0 (hv 0 (str"Case" ++ brk (1,1) ++
             debug_print c ++ spc () ++
             str "instance" ++ brk (1,1) ++ Instance.pr Quality.QVar.raw_pr Univ.(Universe.pr Level.raw_pr) _u ++ spc () ++
  ...
  | Array(u,t,def,ty) -> str"Array(" ++ prlist_with_sep pr_comma debug_print (Array.to_list t) ++ str" | "
      ...
      ++ str")@{" ++ UVars.Instance.pr Quality.QVar.raw_pr (Univ.Universe.pr Univ.Level.raw_pr) u ++ str"}"
