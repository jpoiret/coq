...
let interp_assumption ~program_mode ~sort_poly env sigma impl_env bl c =
  ...
  let sigma, (impls, ((env_bl, ctx), impls1, _locs)) = interp_context_evars ~program_mode ~unconstrained_sorts:sort_poly ~impl_env env sigma bl in
...
let empty_mono_univ_entry =
  UState.{ universes_entry_universes = Monomorphic_entry PolyConstraints.ContextSet.empty;
    universes_entry_binders = UnivNames.empty_binders }
...
let interp_context_gen scope ~program_mode ~sort_poly ~kind ~autoimp_enable ~coercions env sigma l =
  let initial = sigma in
  let sigma, (ienv, ((env, ctx), impls, locs)) = interp_named_context_evars ~program_mode ~unconstrained_sorts:sort_poly ~autoimp_enable env sigma l in
  ...
  let sigma, ctx = Evarutil.finalize ~to_type:(not sort_poly) ~partial:true sigma @@ fun nf ->
...
let do_assumptions ~program_mode ~poly ~sort_poly ~cumulative ~scope ~kind ?user_warns ~inline l =
  ...
  let env = Global.env () in
  let sigma, udecl = interp_cumul_poly_decl_opt env udecl in
  let coercions, ctx = local_binders_of_decls ~poly l in
  let sigma, ctx = interp_context_gen scope ~program_mode ~sort_poly ~kind ~autoimp_enable:true ~coercions env sigma ctx in
...
let do_context ~program_mode ~poly ~sort_poly ctx =
  let sec = Lib.sections_are_opened () in
  ...
  in
  let sigma, ctx = interp_context_gen scope ~program_mode ~sort_poly ~kind:Context ~autoimp_enable:false ~coercions:Id.Set.empty env sigma ctx in
...
let interp_context env sigma ctx =
  ...
  let sigma, ctx = interp_context_gen Locality.Discharge ~program_mode:false ~sort_poly:false ~kind:Context ~autoimp_enable:false ~coercions:Id.Set.empty env sigma ctx in
