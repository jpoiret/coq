...
Class DefaultRelation@{s s' | u v|} A (R : relation@{s s' | u v} A).
Register DefaultRelation as rewrite.DefaultRelation.

(** To search for the default relation, just call [default_relation]. *)

Definition default_relation@{s s' | u v|} `{DefaultRelation@{s s' | u v} A R} := R.

(** Every [Equivalence] gives a default relation, if no other is given
  (lowest priority). *)

#[global]
Instance equivalence_default@{s s' | u v|} `(Equivalence@{s s' | u v} A R) : DefaultRelation R | 4.
Defined.
