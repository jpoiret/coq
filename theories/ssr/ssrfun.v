...
Require Import ssreflect Sigma.
...
Variables (I : Type@{s|l}) (i : I)
          (T_ : I -> Type@{s|lT}) (U_ : I -> Type@{s|lU}).

Definition tag@{s|l l'|} A P := @fst@{s |l l'} A P.
...
Lemma s2valP u : P (s2val u). Proof. case: u => x p; by case p. Qed.

Lemma s2valP' u : Q (s2val u). Proof. case: u => x p; by case p. Qed.
...
Proof. intro s. unshelve esplit; intro i.
  ...
  - cbn. by destruct (s i).
Qed.
(* ** old proof ** by case/all_tag=> f; exists f. Qed.*)
