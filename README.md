...
- New vernacular to explicitly define global elimination constraints between sorts, e.g. `Constraint s ~> s'`. (for global sorts declared with `Sort s s'`.) to define a global constraint between sort `s` and `s'`.
